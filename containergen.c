...
static void emit_vector_type(FILE* out, const char* T) {
    ...
    // cvec_T_init - initialize an empty vector
    fprintf(out,
            "static inline void cvec_%s_init(cvec_%s* v) {\n"
            "    v->data = NULL;\n"
            "    v->length = 0;\n"
            "    v->capacity = 0;\n"
            "}\n\n",
    ...
    // cvec_T_pop - remove and return last element
    fprintf(out,
    ...
    // cvec_T_peek - get last element without removing
    fprintf(out,
    ...
    // cvec_T_get - get pointer to element at index
    fprintf(out,
    ...
    // cvec_T_set - set element at index
    fprintf(out,
    ...
    // cvec_T_clear - reset length without freeing
    fprintf(out,
    ...
    // cvec_T_reserve - ensure capacity for at least n elements
    fprintf(out,
            "static inline void cvec_%s_reserve(cvec_%s* v, size_t n) {\n"
            "    if (n > v->capacity) {\n"
            "        v->data = (%s*)realloc(v->data, n * sizeof(%s));\n"
            "        v->capacity = n;\n"
            "    }\n"
            "}\n\n",
            sanitized_T,
...
static void emit_hashmap_type(FILE* out, const char* K, const char* V) {
    ...
    // Init
    fprintf(out,
            "static inline void cmap_%s_%s_init(cmap_%s_%s* m) {\n"
            "    m->entries = NULL;\n"
            "    m->capacity = 0;\n"
            "    m->count = 0;\n"
            "    m->tombstones = 0;\n"
    ...
    // Forward declare grow (needed by put)
    fprintf(out,
    ...
    // Get - returns pointer to value, or NULL if not found
    fprintf(out,
    ...
    // Clear - remove all entries without freeing
    fprintf(out,
    ...
    // Grow - internal function to resize the table
    fprintf(out,
            "static void cmap_%s_%s_grow(cmap_%s_%s* m) {\n"
            ...
            "    m->entries = (cmap_%s_%s_entry*)calloc(m->capacity, sizeof(cmap_%s_%s_entry));\n"
            "    m->count = 0;\n"
            "    m->tombstones = 0;\n"
            "    \n"
            "    // Reinsert old entries\n"
            "    // Need to do this since the positions depends on capacity (mod the hash by "
            "capacity)\n"
...
static void emit_hashset_type(FILE* out, const char* K) {
    ...
    // Init
    fprintf(out,
            "static inline void cset_%s_init(cset_%s* s) {\n"
            "    s->entries = NULL;\n"
            "    s->capacity = 0;\n"
            "    s->count = 0;\n"
            "    s->tombstones = 0;\n"
    ...
    fprintf(out, "static void cset_%s_grow(cset_%s* s);\n\n", sanitized_K, sanitized_K);

    // Add - insert key if not present
    fprintf(out,
    ...
    fprintf(out,
            ...
            "    s->entries = (cset_%s_entry*)calloc(s->capacity, sizeof(cset_%s_entry));\n"
            "    s->count = 0;\n"
            "    s->tombstones = 0;\n"
            "    \n"
            "    for (size_t i = 0; i < old_cap; i++) {\n"
